...
namespace AOC_3_1
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var pos = segmentB.GetCollisionWith(segmentA);
            var po2 = segmentA.GetCollisionWith(segmentB);
        }

        public static int calculatexT(int xF, string command)
        {
            if (command[0] == 'L')
            {
                return xF - int.Parse(command.Substring(1));
            }
            if (command[0] == 'R')
            {
                return xF + int.Parse(command.Substring(1));
        ...
        public static int calculateyT(int yF, string command)
        {
            if (command[0] == 'D')
            {
                return yF - int.Parse(command.Substring(1));
            }
            if (command[0] == 'U')
            {
                return yF + int.Parse(command.Substring(1));
        ...
        class WireSegment
        {
            ...
            public int calculateDistanceForSegment()
            {
                return total + Math.Abs(xF - xT) + Math.Abs(yF - yT);
            }

            public Position GetCollisionWith(WireSegment wire)
            {
                // Determine direction of wires
                if (wire.xF == wire.xT) // Vertical wire
                {
                    // now we know that we have to take the y values of the wire and compare them with the x value of our own wire
                    if (yF == yT) // Check if our own wire is horizontal
                    {
                        // yF is our y position
                        if (isInRange(yF, wire.yF, wire.yT) && isInRange(wire.xF,xF, xT)) // Collision
                        {
                            return new Position
                            {
                                x = wire.xF,
                                y = yF,
                                DistanceToPositionFromA = total + Math.Abs(wire.xF - xF),
                                DistanceToPositionFromB = wire.total + Math.Abs(wire.yF - yF)
                            };
                        }
                    }
                }

                if (wire.yF == wire.yT) // Horizontal wire
                {
                    if (xF == xT)
                    {
                        // yF is our y position
                        if (isInRange(xF, wire.xF, wire.xT) && isInRange(wire.yF,yF, yT)) // Collision
                        {
                            return new Position
                            {
                                y = wire.yF,
                                x = xF,
                                DistanceToPositionFromA = total + Math.Abs(wire.yF - yF),
                                DistanceToPositionFromB = wire.total + Math.Abs(wire.xF - xF)
                            };
                        }
                    }
                }
        ...
        class Position
        {
            ...
            public int DistanceToPositionFromA { get; set; }
            public int DistanceToPositionFromB { get; set; }
        }

        static IEnumerable<WireSegment> getWireSegments(IEnumerable<string> split)
        {
            var previousSegment = new WireSegment();

            var segments = new List<WireSegment>();
            foreach (var b in split)
            {
                var ws = new WireSegment
                {
                    xF = previousSegment.xT,
                    yF = previousSegment.yT,
                    xT = calculatexT(previousSegment.xT, b),
                    yT = calculateyT(previousSegment.yT, b),
                    total = previousSegment.calculateDistanceForSegment()
                };
                previousSegment = ws;
                segments.Add(ws);
            }
        ...
        static bool isInRange(int num, int r1, int r2)
        {
